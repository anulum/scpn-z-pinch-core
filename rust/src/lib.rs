...
//! the Coulomb logarithm is an input. The geometry kernels of the device
//! 3D model (unit circle, tessellation, mesh measures) live in the shared
//! kernel library `scpn-reactor-kernels` and its native crate, which the
//! device repository pins (ADR 0007); this crate carries physics only.
//! Nothing here solves an equation and no value describes a real machine;
//! the physics design record is ADR 0005 of the repository.
